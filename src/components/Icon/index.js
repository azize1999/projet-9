/* istanbul ignore file */ // Directive pour que ce fichier soit ignoré dans la couverture des tests
import PropTypes from "prop-types";

// Composant Icon qui prend un nom en tant que prop pour afficher différentes icônes SVG
const Icon = ({ name }) => {
  let icon; // Variable pour stocker l'élément SVG correspondant au nom fourni

  // Utilisation de switch pour définir l'icône en fonction de name
  switch (name) {
    case "twitch":
      icon = (
        <path
          data-testid="icon" // Utilisé pour faciliter le ciblage dans les tests
    ...
    case "facebook":
      icon = (
        <path
          data-testid="icon" // Changement : ajout de data-testid pour faciliter les tests
    ...
    case "close":
      ...
      break;
    default:
      icon = null // Par défaut, aucun icône n'est affiché si name n'est pas reconnu
  }

  // Retourne le SVG avec l'icône sélectionnée
  return (
    <svg
      width="36"
      height="36"
    ...
    >
      {icon} {/* Contient le chemin de l'icône */}
    </svg>
  );
};

Icon.propTypes = {
  name: PropTypes.string.isRequired, // Définit le type et l'obligation de la prop name
}
